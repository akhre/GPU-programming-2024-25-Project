...
filter1_handler = logging.FileHandler("filter_laplacian.log", "w")
filter2_handler = logging.FileHandler("filter_gaussian_blur.log", "w")
filter3_handler = logging.FileHandler("filter_bicubic_interpolation.log", "w")
filter4_handler = logging.FileHandler("filter_bilateral.log", "w")
...
class Filter4Frame(VideoFrame.VideoFrame):
    ...
    def apply_filter(self, frame, d_frame):
        ...
        # Measure distortion
        mse, psnr = measure_distortion(frame, filtered_frame)   # Utile in questo caso?
        
        print(f"Filter 4 - MSE: {mse}, PSNR: {psnr}")

        # Log the results
        filter4_logger.info(f"Timestamp: {datetime.now()}, EXECUTION TIME ms: {elapsed_time_ms}, MSE: {mse}, PSNR: {psnr}")
